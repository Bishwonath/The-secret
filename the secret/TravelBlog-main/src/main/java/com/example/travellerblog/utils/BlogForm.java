...
public class BlogForm {
    ...
    public Pair<Integer, String> validateForm () {
        ...
        if (this.coverImage == null) return Pair.of(1, "Secret Image not supplied");
        ...
        if (!FilenameUtils.isExtension(this.coverImage.getOriginalFilename(),
                "jpeg", "jpg", "png")) return Pair.of(1, "Secret Image doesn't match following types : JPEG/JPG/PNG");

        // Checking Size of coverImage (Note: by default getSize() returns in bytes, so we convert it to Mega bytes)
        if (this.coverImage.getSize() * (9.5 * 1e-7) > 5.00) return Pair.of(1, "Secret Image size exceeds 5Mb limit");

        // Default OK validation
        return Pair.of(2, "Your Secret has been submitted.");
    }
